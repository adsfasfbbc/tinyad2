...
DATASET="visa"
# Replace with your local VisA root path before running.
TRAIN_DATA_PATH="<REPLACE_WITH_YOUR_VISA_PATH>"
DEVICE="cuda:0"

if [[ "${TRAIN_DATA_PATH}" == *"REPLACE_WITH_YOUR_VISA_PATH"* ]]; then
  echo "Please set TRAIN_DATA_PATH to your real VisA path in scripts/run_timm_visa.sh"
  exit 1
fi
