...
class HybridSyntheticAnomaly:
    """50/50 hybrid of Perlin and CutPaste."""
    ...
    def _cutpaste(self, image: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if random.random() < 0.5:
            ...
            patch = F.interpolate(patch.unsqueeze(0), size=(ph, pw), mode="bilinear", align_corners=False).squeeze(0)
    ...
    def _perlin(self, image: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ref = torch.clamp(image + 0.25 * (2.0 * noise_rgb - 1.0), min=image.min(), max=image.max())
        out = image * (1.0 - mask) + ((1.0 - self.blend_alpha) * image + self.blend_alpha * ref) * mask
        return out, mask
